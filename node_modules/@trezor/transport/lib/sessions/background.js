...
class SessionsBackground extends utils_2.TypedEmitter {
    ...
    enumerateDone(payload) {
        const disconnectedDevices = Object.keys(this.descriptors).filter(pathInternal => !payload.descriptors.find(d => d.path === pathInternal));
    ...
    getPathBySession({ session }) {
        const path = Object.keys(this.descriptors).find(pathKey => { var _a; return ((_a = this.descriptors[pathKey]) === null || _a === void 0 ? void 0 : _a.session) === session; });
        if (!path) {
    ...
    getInternal(pathPublic) {
        return Object.keys(this.pathInternalPathPublicMap).find(internal => this.pathInternalPathPublicMap[internal] === pathPublic);
    }
    dispose() {
